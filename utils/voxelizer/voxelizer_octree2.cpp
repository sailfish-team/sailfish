...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    cerr << "Usage: ./voxelizer <STL file> <output_base> [voxel size]" << endl << endl;
    return -1;
  }

  std::string output_fname(argv[2]);

  const double voxel_size = atof(argv[3]);
  ...
  num_voxels = int(max_span/voxel_size);
  std::cout << "Voxel size: " << voxel_size << " max_span in voxels: " << num_voxels <<  endl;
  ...
  OctreeToMatrix(octree, md, pmin, pmax, 0, &voxels);
