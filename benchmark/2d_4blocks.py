...
class BenchmarkXConnGeometry(LBGeometry2D):
    def blocks(self, n=None):
        ...
        blocks = [SubdomainSpec2D((0, 0), (w, self.gy)),
                  ...
                  SubdomainSpec2D((2*w, 0), (w, self.gy)),
                  SubdomainSpec2D((3*w, 0), (w, self.gy))]
        return blocks
...
class BenchmarkYConnGeometry(LBGeometry2D):
    def blocks(self, n=None):
        ...
        blocks = [SubdomainSpec2D((0, 0), (self.gx, h)),
                  ...
                  SubdomainSpec2D((0, 2*h), (self.gx, h)),
                  SubdomainSpec2D((0, 3*h), (self.gx, h))]
        return blocks


class BenchmarkXYConnGeometry(LBGeometry2D):
    def blocks(self, n=None):
        w = self.gx / 2
        h = self.gy / 2
        blocks = [SubdomainSpec2D((0, 0), (w, h)),
                  SubdomainSpec2D((0, h), (w, h)),
                  SubdomainSpec2D((w, 0), (w, h)),
                  SubdomainSpec2D((w, h), (w, h))]
        return blocks
...
def run_benchmark(boundary_split=True, suffix=''):
    ...
    np.savetxt('2d_4blocks_x{0}.dat'.format(suffix), summary, fmt)
    util.save_timing('2d_4blocks_x{0}.timing'.format(suffix), timings)
    ...
    np.savetxt('2d_4blocks_y{0}.dat'.format(suffix), summary, fmt)
    util.save_timing('2d_4blocks_y{0}.timing'.format(suffix), timings)
    ...
    w = mem_size / node_cost / h
    sizes = [(x, y) for x, y in zip(w, h)]
    test_sizes(sizes, BenchmarkXYConnGeometry)
    np.savetxt('2d_4blocks_xy{0}.dat'.format(suffix), summary, fmt)
    util.save_timing('2d_4blocks_xy{0}.timing'.format(suffix), timings)
