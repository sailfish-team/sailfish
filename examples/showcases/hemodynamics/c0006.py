...
class C0005Subdomain(common.InflowOutflowSubdomain):
...
class C0005Sim(common.HemoSim):
    subdomain = C0005Subdomain
    phys_diam = 0.0019868 * 2
    lb_v = 0.001

    @classmethod
    def update_defaults(cls, defaults):
        super(C0005Sim, cls).update_defaults(defaults)
        defaults.update({
            'max_iters': 500000,
    ...
    def set_walls(cls, config):
        ...
        if not config.base_name:
            config.base_name = 'results/re%d_c0005_%d_%s' % (
    ...
    def get_diam(cls, config):
        return 2.0 * np.sqrt(np.sum(np.logical_not(config._wall_map[:,0,:])) /
                             np.pi)

    @classmethod
    def modify_config(cls, config):
        cls.set_walls(config)
        super(C0005Sim, cls).modify_config(config)
